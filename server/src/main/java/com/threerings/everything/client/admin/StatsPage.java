...
/**
 * An admin page for viewing stats and editing players.
 */
public class StatsPage extends DataPanel<AdminService.StatsResult>
{
    ...
    protected void init (final AdminService.StatsResult result)
    {
        final FluentTable contents = new FluentTable(10, 0);
        add(contents);

        // set up our thing database stats
        FlowPanel stats = new FlowPanel();
        stats.add(Widgets.newLabel("Thing Stats", "machine"));
        stats.add(Widgets.newLabel("Total things: " + result.stats.totalThings));
        stats.add(Widgets.newLabel("Total categories: " + result.stats.totalCategories));
        stats.add(Widgets.newLabel("Total players: " + result.stats.totalPlayers));
        stats.add(Widgets.newLabel("Total cards: " + result.stats.totalCards));
        ...
        contents.at(0, 0).setRowSpan(2).setWidget(stats).alignTop();
        ...
        contents.at(0, 2).setRowSpan(2).alignTop().setWidgets(
        ...
        contents.at(0, 3).alignTop().setWidgets(Widgets.newLabel("Find Player", "machine"), search);
